...
$SENTIEON_INSTALL_DIR/bin/sentieon driver -r $FASTA -t $NT -i tumor_deduped.bam -i normal_deduped.bam ${INTERVAL_FILE:+--interval_padding 250 --interval $INTERVAL_FILE} -q tumor_recal_data.table -q normal_recal_data.table --algo TNscope --tumor_sample $TUMOR_SM --normal_sample $NORMAL_SM --dbsnp $KNOWN_DBSNP --sv_mask_ext 10 --max_fisher_pv_active 0.05 --min_tumor_allele_frac 0.005 --assemble_mode 4 output_tnscope.pre_filter.vcf.gz
