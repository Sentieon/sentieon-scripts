...
$SENTIEON_INSTALL_DIR/bin/sentieon driver -r $FASTA -t $NT -i tumor_deduped.bam \
    ...
    --dbsnp $KNOWN_DBSNP --sv_mask_ext 10 --min_tumor_allele_frac 0.03 \
...
# ******************************************
( $BCFTOOLS_BINARY annotate -x "FILTER/triallelic_site" output_tnscope.pre_filter.vcf.gz || echo "VCF filtering failed"; exit 1 ) | \
    ( $BCFTOOLS_BINARY filter -m + -s "insignificant" -e "(PV>0.25 && PV2>0.25) || (INFO/STR == 1 && PV>0.05)" || echo "VCF filtering failed"; exit 1 ) | \
        ( $BCFTOOLS_BINARY filter -m + -s "orientation_bias" -e "FMT/FOXOG[0] == 1" || echo "VCF filtering failed"; exit 1 ) | \
        ( $BCFTOOLS_BINARY filter -m + -s "strand_bias" -e "SOR > 3" || echo "VCF filtering failed"; exit 1 ) | \
        ( $BCFTOOLS_BINARY filter -m + -s "low_qual" -e "QUAL < $MIN_QUAL" || echo "VCF filtering failed"; exit 1 ) | \
        ( $BCFTOOLS_BINARY filter -m + -s "short_tandem_repeat" -e "RPA[0]>=10" || echo "VCF filtering failed"; exit 1 ) | \
        ( $BCFTOOLS_BINARY filter -m + -s "noisy_region" -e "ECNT>5" || echo "VCF filtering failed"; exit 1 ) | \
        ( $BCFTOOLS_BINARY filter -m + -s "read_pos_bias" -e "FMT/ReadPosRankSumPS[0] < -8" || echo "VCF filtering failed"; exit 1 ) | \
