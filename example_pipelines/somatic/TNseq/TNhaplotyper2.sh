...
START_DIR="$PWD/test/TNhaplotyper2" #Determine where the output files will be stored
...
# ******************************************
$SENTIEON_INSTALL_DIR/bin/sentieon driver -r $FASTA -t $NT -i tumor_deduped.bam -i normal_deduped.bam -q tumor_recal_data.table -q normal_recal_data.table --algo TNhaplotyper2 --tumor_sample $TUMOR_SM --normal_sample $NORMAL_SM ${PON:+--pon $PON} ${GERMLINE_VCF:+--germline_vcf $GERMLINE_VCF} output-tnhap2-tmp.vcf.gz --algo OrientationBias --tumor_sample $TUMOR_SM output-orientation --algo ContaminationModel --tumor_sample $TUMOR_SM --normal_sample $NORMAL_SM ${CONTAMINATION_VCF:+--vcf $CONTAMINATION_VCF} --tumor_segments output-contamination-segments output-contamination
$SENTIEON_INSTALL_DIR/bin/sentieon driver -r $FASTA --algo TNfilter -v output-tnhap2-tmp.vcf.gz --tumor_sample $TUMOR_SM --normal_sample $NORMAL_SM --contamination output-contamination --tumor_segments output-contamination-segments --orientation_priors output-orientation output-tnhap2.vcf.gz
